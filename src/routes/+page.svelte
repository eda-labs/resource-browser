<script lang="ts">
  ...
  const resourceStore = writable<string[]>(resources)
  const resourceSearch = writable("")

  const resourceSearchFilter = derived([resourceSearch, resourceStore], ([$resourceSearch, $resourceStore]) => 
    $resourceStore.filter(x => $resourceSearch.split(/\s+/).every(y => x.includes(y))))
</script>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 items-center gap-5">
      <div>
        ...
        <div class="text-gray-300 mt-2 max-w-[500px] text-sm">
          <p>View Custom Resource Definition (CRD)</p>
        </div>
					...
					<ul>
						{#each $resourceSearchFilter as resource, i}
							<li class="items-center text-gray-900 hover:bg-gray-200 {i > 0 ? 'border-t border-gray-300 dark:border-gray-600' : ''} dark:hover:bg-gray-700">
								<a href={resource}>
									<p class="px-4 py-3 text-sm dark:text-white overflow-x-auto scroll-light dark:scroll-dark">{resource}</p>
								</a>
							</li>
						{/each}
					</ul>
